...
type
  ...
  published
    //информация по актуальным остаткам товара на складах
  ...
  private
    ...
    procedure Setsklad_name( AValue:String);
  ...
  published
    ...
    //наименование склада (на случай, если в ТСД нет данных по складу)
    property sklad_name:String read Fsklad_name write Setsklad_name;
    //количество в базовых ед. измерения для номенклатуры
...
procedure TTable_Record.Setsklad_name(AValue: String);
begin
  Fsklad_name:=AValue;
  ModifiedProperty('sklad_name');
end;

procedure TTable_Record.Setquantity(AValue: String);
begin
  Fquantity:=AValue;
...
begin
  ...
  P:=RegisterProperty('sklad_name', 'sklad_name', [], '', -1, -1);
